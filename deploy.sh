...
# RAMA Chat App - AWS EC2 Deployment Script
# =========================================

set -e  # Exit on any error

echo "🚀 Starting RAMA Chat App Deployment on AWS EC2..."
...
if [ -z "$EC2_PUBLIC_IP" ]; then
    print_error "Could not get EC2 public IP. Make sure you're running this on an EC2 instance."
    exit 1
fi
...
cp -r /home/$USER/chat-app/* /var/www/rama-chat/
...
# API Configuration (Single Port - No CORS issues!)
...
# CORS Configuration (Single Port - Minimal CORS needed)
CORS_ORIGINS=http://$EC2_PUBLIC_IP
CORS_CREDENTIALS=true
CORS_METHODS=GET,POST,PUT,PATCH,DELETE,OPTIONS
CORS_HEADERS=Content-Type,Authorization

# Socket.io Configuration (Single Port)
SOCKET_CORS_ORIGINS=http://$EC2_PUBLIC_IP
...
FILE_BASE_URL=http://$EC2_PUBLIC_IP
...
VITE_FRONTEND_URL=http://$EC2_PUBLIC_IP
VITE_SOCKET_URL=http://$EC2_PUBLIC_IP
...
# Configure Nginx (Simplified for single port)
print_status "Configuring Nginx for single port deployment..."
sudo tee /etc/nginx/sites-available/rama-chat << EOF
server {
    listen 80;
    server_name $EC2_PUBLIC_IP;

    # Proxy everything to Node.js app (single port)
    location / {
...
# Enable the site
sudo ln -sf /etc/nginx/sites-available/rama-chat /etc/nginx/sites-enabled/
...
print_status "Your RAMA Chat App is now running at: http://$EC2_PUBLIC_IP"
print_status "Frontend & Backend: http://$EC2_PUBLIC_IP (Single Port - No CORS issues!)"
print_status "API Endpoints: http://$EC2_PUBLIC_IP/api"
print_status "Health Check: http://$EC2_PUBLIC_IP/health"
print_status "Socket.io: http://$EC2_PUBLIC_IP/socket.io"

print_warning "Important Notes:"
echo "1. Make sure your EC2 Security Group allows inbound traffic on ports 22, 80, and 443"
